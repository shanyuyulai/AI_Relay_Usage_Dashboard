...
普通用户不需要安装 Node.js、npm 或执行构建命令，直接下载已经打包好的 ZIP 即可使用。发布后的压缩包可从 [GitHub Releases 下载页](https://github.com/shanyuyulai/AI_Relay_Usage_Dashboard/releases) 获取，例如 `AI_Relay_Usage_Dashboard_v0.4.0.zip`。
...
在设置页点击站点右侧的 **“🔔 余额警报”**，或进入“添加／编辑站点”配置。每站最多10条规则，可设置名称、原币余额临界值、启用状态和通知保持显示选项。

- 每次成功采集余额后检查；仅当余额从 **高于临界值** 下降至 **临界值及以下** 时发出系统通知。
- 首次有效采集仅建立基准；持续低于阈值不重复通知。充值回升后再次跌破，可以再次提醒。
- 同站同次采集命中多个规则时合并通知。通知独立于“采集通知”档位；关闭普通采集通知不会关闭余额警报。
- 停用插件、站点或规则会停止相应提醒。重新启用或修改阈值后重新建立基准，不补报历史变化。
- 阈值按原币判定，不受人民币显示切换影响。币种改变时需确认并重新启用规则。
- 规则随全量备份迁移，报警基准与投递运行态不迁移；历史导入不会触发警报。
- 可在编辑器中刷新通知权限和最近投递状态。系统通知实际展示受浏览器、操作系统通知设置及免打扰模式影响。

更新代码后请重新构建并在扩展管理页重新加载工程 `dist/`；若仍加载历史分发目录，不会出现新功能入口。
...
- 设置页每站点Switch、标题同一行插件总开关；侧边栏与极简面板不增加开关。
- 每站最多10条余额警报，首次有效采集建立基准；同轮命中合并系统通知。
- 编辑站点改为响应式双列，固定底部操作区；自定义采集和诊断面板新增直接关闭按钮。
- 统一明暗主题滚动条；设置列表、侧边栏和站点详情优先使用网站图标，失败时显示彩色首字。
- **极简面板不显示站点图标**，保留状态圆点、名称和余额。
- 图标采用匿名同源请求，不增加权限或第三方服务；相关隐私说明见PRIVACY.md。

安装包与SHA-256校验文件随GitHub Release提供。完整说明见[RELEASE_NOTES_0.4.0.md](./RELEASE_NOTES_0.4.0.md)。升级前请先导出全量备份，优先在原加载路径替换文件后重新加载扩展，不要为了升级卸载旧扩展。
